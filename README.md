...
- `DualCloudOps`: YC-style LeRobot-to-Qualcomm release pitch for governed routing of one robot training contract across China CUDA/domestic-compliance lanes or overseas hyperscaler/neocloud/HF Jobs lanes, preserving data boundaries, reproducible eval gates, cost/egress evidence, AI Hub/QAIRT/QNN profiles, signed artifacts, SafetyOps gates, and rollbackable Qualcomm edge packages.
- `RevenueStack`: YC-style robot revenue operating-system pitch for turning robot events into recurring revenue: RaaS/leasing, skill packages, cloud training credits, SLA/warranty, channel settlement, fapiao/invoice workflows, usage metering, Qualcomm AI Hub/QNN profile evidence, and production-to-revenue attach.
- `RobotAppLayer`: YC-style trusted robot app release-layer pitch for turning ROS 2 packages, LeRobot policies, signed OCI/.rap skill bundles, physical permissions, compatibility testing, Qualcomm AI Hub/QNN evidence, SkillDock marketplace distribution, RevenueStack billing, and EdgeFleet rollback into an App Store + MDM + CI/CD workflow for robots.
- `OpsConnector`: YC-style edge-verified enterprise robot workflow connector pitch for turning WMS/MES/ERP/LIMS/SCADA/CMMS intent into TaskContracts, local Dragonwing edge execution, human approval, exceptions, Qualcomm AI evidence, audit writeback, and reusable CN/Global workflow packs.
- `LabForgePilot`: YC-style main-demo and first-commercial-wedge pitch for a desktop sample-transfer workcell that turns LeRobotDataset v3 collection, ACT/HIL training, Qualcomm Dragonwing edge profiles, barcode/sample verification, LIMS/ELN/QMS writeback, validation-ready evidence packs, and failure mining into a purchasable lab/QC/education product.
- `EdgeRuntimeBench`: YC-style physical-AI runtime evidence pitch for turning LeRobot/ACT/SmolVLA/VLM policies into CI benchmark runs, Qualcomm runtime matrices, SafetyOps release gates, procurement scorecards, signed release dossiers, SkillDock certification, EdgeFleet canaries, and rollbackable evidence packs.
- `BoardBringupKit`: YC-style 48-hour Qualcomm board-to-robot sprint pitch for turning RB3/QCS6490, Rhino X1/QCS8550, Dragon Q6A/QCS6490, protected 12/24V power, camera/GStreamer, industrial I/O, SafetyOps gates, LeRobot episodes, EdgeRuntimeBench profiles, factory/HIL tests, and bring-up evidence into reusable RobotMac Core candidates.
